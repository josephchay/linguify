...
class Chat:
    ...
    def load_models(self, source: Literal['huggingface', 'local', 'custom'] = 'local', force_redownload=False, custom_path='<LOCAL_PATH>', **kwargs):
        if source == 'local':
    ...
    def _inference(self, text, skip_refine_text=False, refine_text_only=False, params_refine_text={}, params_infer_code={'prompt': '[speed_5]'}, use_decoder=True, do_text_normalization=True, lang=None, stream=False, do_homophone_replacement=True):
        ...
        result_gen = inference_code(self.pretrain_models, text, **params_infer_code, return_hidden=use_decoder, stream=stream)
        if use_decoder:
            field = 'hiddens'
    ...
    def inference(self, text, skip_refine_text=False, refine_text_only=False, params_refine_text={},
              params_infer_code={'prompt': '[speed_5]'}, use_decoder=True, do_text_normalization=True,
              lang=None, stream=False, do_homophone_replacement=True):
        res_gen = self._inference(text, skip_refine_text, refine_text_only, params_refine_text,
                                  params_infer_code, use_decoder, do_text_normalization, lang,
                                  stream, do_homophone_replacement)
        if stream:
            return res_gen
        else:
            return next(res_gen)
    ...
    def sample_random_speaker(self):
        dim = self.pretrain_models['gpt'].model.layers[0].mlp.gate_proj.in_features
    ...
    def init_homophones_replacer(self):
        if self.homophones_replacer:
            ...
            try:
                self.homophones_replacer = HomophonesReplacer(
