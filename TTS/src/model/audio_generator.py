...
class AudioGenerator(nn.Module):
    ...
    def generate(self, emb, inputs_ids, temperature, eos_token: Union[int, torch.Tensor], attention_mask=None,
        ...
        with torch.no_grad():
            ...
            start_idx, end_idx = inputs_ids.shape[1], torch.zeros(inputs_ids.shape[0], device=inputs_ids.device,
                                                                  dtype=torch.long)
                ...
                for i in range(max_new_token):
                    ...
                    if stream:
                        if end_idx % 24 and not finish.all():
                            continue
                        y_inputs_ids = [inputs_ids[idx, start_idx: start_idx + i] for idx, i in
                                        enumerate(end_idx.int())]
