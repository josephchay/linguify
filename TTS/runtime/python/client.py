...
def main():
    with grpc.insecure_channel("{}:{}".format(args.host, args.port)) as channel:
        stub = linguifytts_pb2_grpc.LinguifyTTSStub(channel)
        request = linguifytts_pb2.Request()
        if args.mode == 'sft':
            logging.info('send sft request')
            sft_request = linguifytts_pb2.sftRequest()
            ...
            zero_shot_request = linguifytts_pb2.zeroshotRequest()
            ...
            cross_lingual_request = linguifytts_pb2.crosslingualRequest()
            ...
            instruct_request = linguifytts_pb2.instructRequest()
