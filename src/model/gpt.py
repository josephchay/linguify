...
class GPTWrapper(nn.Module):
    def __init__(self, gpt_config, num_audio_tokens, num_text_tokens, num_vq=4, **kwargs):
        super().__init__()
    ...
    def generate(self,  emb,  inputs_ids,  temperature,  eos_token,  attention_mask = None, max_new_token = 2048,
                 min_new_token = 0, LogitsWarpers = [], LogitsProcessors = [], infer_text=False, return_attn=False, return_hidden=False):
        with torch.no_grad():
            attentions = []
            hiddens = []
